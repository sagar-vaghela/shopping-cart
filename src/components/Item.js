...
class Item extends Component {
  ...
  render() {
    ...
    return (
      ...
      // </div>
      <div>
        <a href="#" className="list-group-item list-group-item-action flex-column align-items-start" key={this.props.item.id}>
          <div className="d-flex w-100 justify-content-between">
              <h5 className="mb-1">{this.props.item.name}</h5>
              <small>Price: {this.props.item.price} <i className="fa fa-inr"></i></small>
          </div>
          <p className="mb-1">
              <img src="https://5.imimg.com/data5/EF/RQ/MY-3030942/lenovo-desktop-computer-500x500.jpg" className="productImgThumbnail" />
              {this.props.item.description}
          </p>
            <Button type="button" className="btn btn-info pull-right" text="Add to cart" id={this.props.item.id} onClick={this.props.handleAddToCart} />
         </a>
      </div>
    );
  }
